...
class Walkthrough_V1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      checkedExchanges: [], // 5
  ...
  renderTextInput(classes, field) {
    console.log('HELLO')
    console.log('in TEXT INPUT!!!', classes, field[0], arguments);
    console.log('this.handleChange(field[0])', this.handleChange(field[0]));
    return (
      <Grid key={this.state.currentSceneNumber} item>
        <TextField
          fullWidth
  ...
  renderBlaBli(classes) {
    let buttonStrings = actions[this.state.currentSceneNumber].strings;
    let argument = actions[this.state.currentSceneNumber].arguments[0];
  ...
  */
  ...
  render() {
    ...
    if (actions[currentSceneNumber]) {
      if (actions[currentSceneNumber].arguments) {
        args = actions[currentSceneNumber].arguments;
      }
    }

    if (
      scenesWithoutUserAction[currentSceneNumber] === undefined
      && this[actions[currentSceneNumber].method] !== undefined
    ) {
      body = this[actions[currentSceneNumber].method](classes, args);
    }
...
export default withStyles(styles)(Walkthrough_V1);
